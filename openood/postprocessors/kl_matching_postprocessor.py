        ...
        if not self.setup_flag:
            ...
            with torch.no_grad():
                for batch in tqdm(id_loader_dict['val'],
                    ...
                    logits = net(data, return_feature=False)
                    all_softmax.append(F.softmax(logits, 1).cpu())

                    # ipdb.set_trace()
                    # print("GH Debug, the shape of softmax is:{}".format(F.softmax(logits, 1).shape))   #  [200, 100] [batch, dimension]
                    # print("GH Debug, the softmax is:{}".format(F.softmax(logits, 1).cpu()))
