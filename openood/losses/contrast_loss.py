...
class CRDLoss(nn.Module):
    ...
    """
    def __init__(self, t_dim, feat_dim, n_data, nce_k=16384, nce_t=0.07, nce_m=0.5):
    ...
    def forward(self, f_i, f_o):
        ...
        # print(f_i.shape) ([768])
        # ipdb.set_trace()
        f_t = torch.cat([f_i.unsqueeze(0), f_o], dim=0)
        # f_s = self.embed_s(f_s)
        f_t = self.embed_t(f_t)
        # print(f_t.shape)
        # data_pair = f_t
        # data_pair[0] = f_s
        # torch.transpose(f_t, 0, 1)
        # out_s, out_t = self.contrast(f_s, f_t, idx, contrast_idx)
        # s_loss = self.criterion_s(f_t)
        loss = self.criterion_s(f_t)
        print(loss)
