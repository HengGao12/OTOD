...
from openood.postprocessors import RankFeatPostprocessor
from openood.utils import Config

from .base_evaluator import BaseEvaluator
from .metrics import compute_all_metrics
# from openood.datasets import get_id_ood_dataloader
...
class OODEvaluator(BaseEvaluator):
    def __init__(self, config: Config):
        ...
        self.id_conf = None
        self.id_gt = None
        # self.id_dataloader_dict = get_dataloader(config)
        # save the scores
        # self.metrics = {
        #     'ood': {
        #         'val': None,
        #         'near':
        #         {k: None
        #          for k in self.id_dataloader_dict['ood']['near'].keys()},
        #         'far': {k: None
        #                 for k in self.id_dataloader_dict['ood']['far'].keys()},
        #     }
        # }
    ...
    def eval_ood(self,
        ...
        # id_pred, id_conf, id_gt, id_feat = postprocessor.inference(
        #     net, id_data_loaders['test'])
        id_pred, id_conf, id_gt = postprocessor.inference(
            net, id_data_loaders['test'])
        # ipdb.set_trace()
        # id_conf_array = np.array(id_conf)
        # print("ID conf shape:{}".format(id_conf_array.shape))
        # np.save('/home1/gaoheng/gh_workspace/OAML/viz/ebo/cifar10/oaml_new_submission_id_conf_' +  dataset_name + '.npy', id_conf)
    ...
    def _eval_ood(self,
        ...
        # print("The shape of id_conf:{}".format(id_conf.shape))
        metrics_list = []
        for dataset_name, ood_dl in ood_data_loaders[ood_split].items():
            # print("Current dataset:{}".format(dataset_name))
            print(f'Performing inference on {dataset_name} dataset...',
                  flush=True)
            # ood_pred, ood_conf, ood_gt, ood_feat = postprocessor.inference(net, ood_dl)
            ood_pred, ood_conf, ood_gt = postprocessor.inference(net, ood_dl)
            # print(ood_conf)
            # ood_conf_array = np.array(ood_conf)
            # np.save('/home1/gaoheng/gh_workspace/OAML/viz/ebo/cifar10/oaml_new_submission_ood_conf_' +  dataset_name + '.npy', ood_conf_array)
            # print("GH debug, the shape of ood conf:{}".format(ood_conf.shape))  # (9000, 512)
            # ipdb.set_trace()
            
            # ood_conf_array = np.array(ood_conf)
            # np.save('/home1/gaoheng/gh_workspace/OAML/viz/ebo/cifar100/oe_ood_conf_' +  dataset_name + '.npy', ood_conf_array)
            # ipdb.set_trace()
            ood_gt = -1 * np.ones_like(ood_gt)  # hard set to -1 as ood
            # print("ood gt:{}".format(ood_gt))
            if self.config.recorder.save_scores:
                self._save_scores(ood_pred, ood_conf, ood_gt, dataset_name)
            # print("The id_conf:{}".format(id_conf))
            # print("ood conf shape:{}".format(ood_conf.shape))
        ...
        # np.save(metrics_list, ood_split)
    ...
    def eval_acc(self,
        ...
        self.id_pred, self.id_conf, self.id_gt = postprocessor.inference(
            net, data_loader)  # The original code
        
        # self.id_pred, self.id_conf, self.id_gt, self.id_feat = postprocessor.inference(
        #     net, data_loader)
        # self.id_feat = normalizer(self.id_feat)
        # np.save('./wrn-28-id-feat-list-cifar10-for-empirical-validation-wo-unitization.npy', self.id_feat)  # used for feature distribution visualization
        # np.save('./wrn-28-id-label-list-cifar10-for-empirical-validation-wo-unitization.npy', self.id_gt)
    ...
    def hyperparam_search(
        ...
        for hyperparam in hyperparam_combination:
            ...
            id_pred, id_conf, id_gt = postprocessor.inference(      # original code
                net, id_data_loader)
            # id_pred, id_conf, id_gt, id_feat = postprocessor.inference(
            #     net, id_data_loader)
            ood_pred, ood_conf, ood_gt = postprocessor.inference(
                net, ood_data_loader)
            # ood_pred, ood_conf, ood_gt, ood_feat = postprocessor.inference(
            #     net, ood_data_loader)
            ood_gt = -1 * np.ones_like(ood_gt)  # hard set to -1 as ood
            pred = np.concatenate([id_pred, ood_pred])
            conf = np.concatenate([id_conf, ood_conf])
