...
from .ood_distill_trainer import OODDistillTrainer
from .generative_ood_distill_trainer import GenerativeOODDistillTrainer
...
def get_trainer(net, train_loader: DataLoader, val_loader: DataLoader,
    ...
    if type(train_loader) is DataLoader:
        trainers = {
            ...
            'ood_distill_trainer': OODDistillTrainer,
            'generative_ood_distill_trainer': GenerativeOODDistillTrainer
        }
...
def get_trainer2(net, train_loader: DataLoader, train_loader2: DataLoader, val_loader: DataLoader,
                config: Config):
    if type(train_loader) is DataLoader:
        trainers = {
            'base': BaseTrainer,
        ...
        if config.trainer.name in ['cider', 'npos']:
            return trainers[config.trainer.name](net, train_loader, train_loader2, val_loader,
                                                 config)
        else:
            return trainers[config.trainer.name](net, train_loader, train_loader2, config)
        ...
        return trainers[config.trainer.name](net, train_loader[0],
                                             train_loader[1], train_loader2, config)
