...
class GenerativeOODDistillTrainer:
    def __init__(self, net: nn.Module, train_loader: DataLoader, train_loader2: DataLoader,
        ...
        if self.dataset == 'cifar10':
            ...
            if self.deep_ood_distill:
                # self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding_cifar10.pt')   # (50, 3, 16384)
                self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding_cifar10_plus.pt')  # (100, 3, 64, 64)
                # self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding_cifar10_plus-1.pt')  # (100, 3, 64, 64)
                # self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding_cifar10_plus-2.pt')  # (100, 3, 64, 64)
                n, b, h, w= self.deep_ood_samples.shape
                # self.deep_ood_samples = self.deep_ood_samples.reshape(n*b, 16384)  # (150, 16384)
                # self.deep_ood_samples = self.deep_ood_samples.reshape(n*b, 128, 128)  # (150, 16384)
                ...
                if self.arch == 'res50':
                    ...
                    # self.mi_loss2 = DEEP_CLUB(16384, 512, 512).cuda()
                    # self.mi_loss2 = DEEP_CLUB(128, 512, 512)
                    # if self.deep_ood_ablation:
                    self.mi_loss2 = DEEP_CLUB(64, 512, 512).cuda()    
                    # else:
                        # now goes the third version
            ...
            if self.ood_fea_distill:
                ...
                # self.cl = CRDLoss(768, 512, self.n_data).cuda()
            if self.deep_ood_distill:
                # self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding.pt')  original
                self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding_cifar100_plus.pt').cuda()    # new outliers: [100, 3, 64, 64] 100 iter, 3 batch, 64 width, 64 height
                # self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding_cifar100_plus-1.pt').cuda()
                # self.deep_ood_samples = torch.load('/home1/gaoheng/gh_workspace/OAML/outlier-generation/deep_ood_embedding_cifar100_plus-2.pt').cuda()
                # n, b, d= self.deep_ood_samples.shape  original
                n, b, h, w = self.deep_ood_samples.shape
                # self.deep_ood_samples = self.deep_ood_samples.reshape(n*b, 128, 128)  # (150, 16384)  # new 1
                ...
                if self.arch == 'res50':
                    ...
                    # self.mi_loss2 = DEEP_CLUB(16384, 512, 512).cuda()
                    # self.mi_loss2 = DEEP_CLUB(128, 512, 512).cuda()  # version 2
                    # now goes the third version
                    self.mi_loss2 = DEEP_CLUB(64, 512, 512).cuda()
        ...
        if self.deep_ood_ablation:
            ...
            self.deep_ood_samples = torch.mean(self.deep_ood_samples, dim=1).squeeze(1)
            self.dt = DT(in_dim=64, out_dim=512).cuda()
        ...
        if self.fea_distill:
            ...
            self.optimizer = torch.optim.SGD(
                [
                    ...
                    {
                        'params': self.logistic_regression.parameters(), 'lr': 0.01
                    }
                    # {
                    #     'params': self.mi_loss.p_mu.parameters(), 'lr': 0.0001
                    # },
    ...
    def train_epoch(self, epoch_idx):
        ...
        for train_step in tqdm(range(1,
            ...
            # print("GH debug feature shape:{}".format(feature.shape))
            ...
            if self.deep_ood_ablation:
                ...
                Ec_out = torch.logsumexp(self.dt(self.deep_ood_samples[idx2]), dim=1)
                Ec_in = torch.logsumexp(feature, dim=1)
                # print("GH, the Ec_out shape:{}".format(Ec_out.shape))
                # print("GH, the feature length:{}".format(len(feature)))  128
                binary_labels = torch.ones((len(feature)+self.n_data)).cuda()
                binary_labels[len(feature):] = 0

                input_for_lr = torch.cat((Ec_in, Ec_out), -1)
                criterion = torch.nn.CrossEntropyLoss()
                output1 = self.logistic_regression(input_for_lr.reshape(-1, 1))
                energy_reg_loss = criterion(output1, binary_labels.long())

                loss += 2.5 * energy_reg_loss
            ...
            if self.ood_fea_distill:
                ...
                noise1 = torch.randn_like(cdl) * 0.1  # used for empirically shown our Theorem 2
                loss += 0.1*cdl + noise1.cuda()
            
            if self.deep_ood_distill:
                idx2 = torch.randperm(self.N2)[:self.n_data]
                selected_ood_samples2 = self.deep_ood_samples[idx2]
                deep_cdl = self.mi_loss2.forward(selected_ood_samples2, feature)
                noise2 = torch.randn_like(deep_cdl) * 0.1  # used for empirically shown our Theorem 2
                loss += 0.2*deep_cdl + noise2.cuda()
