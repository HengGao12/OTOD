...
import timm
from timm.models.vision_transformer import VisionTransformer   # modified
from transformers import ViTFeatureExtractor, ViTModel, ViTConfig, ViTForImageClassification
...
def get_network(network_config):
    ...
    if hasattr(network_config, 'modification') and network_config.modification == 't2fnorm':
        ...
        # modified
        net = VisionTransformer(num_classes=10)  # when cifar-10 use 10, when cifar-100 use 100
        # net.head = nn.Linear(net.head.in_features, 10)
        # net.load_state_dict(torch.load(network_config.checkpoint),
        #                         strict=False)
    elif network_config.name == 'vit_pretrained':
        net = timm.create_model('vit_base_patch16_224.augreg2_in21k_ft_in1k', pretrained=False)
        
    elif network_config.name == 'vit_cifar100':
        vit_config = ViTConfig()
        # net = ViTModel(vit_config)
        net = ViTForImageClassification(vit_config)
        # # feature_extractor = ViTFeatureExtractor.from_pretrained('google/vit-base-patch16-224-in21k')
        # net = ViTModel.from_pretrained('edumunozsala/vit_base-224-in21k-ft-cifar100')
    ...
    if network_config.pretrained:
        if type(net) is dict:
            ...
            net.head = nn.Linear(net.head.in_features, network_config.num_classes)
